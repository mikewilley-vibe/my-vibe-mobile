...
export default function Detail(){
 ...
 {nativeStatus==='present'&&linked?<Button title="✓ In Calendar · Open" disabled={busy} onPress={()=>void openCalendar(linked).catch(problem)}/>:<Button title={busy?'Please wait…':Platform.OS==='ios'?'Add to Apple Calendar':'Add to device calendar'} disabled={busy||locked} onPress={()=>void prepare()}/>}
 {!!choices.length&&<Card><Text style={styles.heading}>Choose a calendar</Text><Text style={styles.body}>Tap a calendar to add this plan.</Text>{choices.map(c=><Button key={c.id} title={`${c.title}${c.source?.name?' · '+c.source.name:''}`} disabled={busy} onPress={()=>void add(c.id)}/>)}<Button title="Cancel" disabled={busy} onPress={()=>setChoices([])}/></Card>}
